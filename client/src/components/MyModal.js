...
const MyModal = ({ setIsShowModal, content, data, handleSelect, setDistrict, setProvince, setCategory }) => {
    ...
    return (
        <>
            <Modal centered show={true} onHide={() => handleClose()} dialogClassName="my-modal">
                <Modal.Header closeButton>
                    <FaArrowLeft fontSize={'20px'} cursor={'pointer'} onClick={handleBack} />
                    <Modal.Title className="w-100 text-center">{content}</Modal.Title>
                </Modal.Header>
                <Modal.Body className="modal-body">
                    {content !== 'Giá' && content !== 'Diện tích' && <MyTable data={data} columns={columns} />}
